...
static void write_tar_modify_header(const char* filename, struct tar_t* header, size_t size)
{
...
static int strat_nasty_structure_header(const char* target){
    int crash_count = 0;
    ...
    for (int i = 0; i < count; i++) {
        ...
        write_tar_modify_header("archive.tar", &header, test);

        if (run_target(target) == 1) {
            char crash_name[PATH_MAX];
            snprintf(crash_name, sizeof(crash_name),
                     "crashes/crash_structure_header_%ld.tar", test);
